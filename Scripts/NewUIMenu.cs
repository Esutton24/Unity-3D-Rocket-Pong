...
public class NewUIMenu : MonoBehaviour
{
    ...
    public void SetActive(NewUIMenu prevMenu)
    {
    ...
    protected virtual void Navigate(Direction moveDirection)
    {
        ...
        if(!_currentHighlightedGO.Equals(_currentSelectable.gameObject))
        {
            //Debug.Log($"Highlighting current\n{_currentHighlightedGO.name} -> {_currentSelectable.gameObject.name}", _currentHighlightedGO);
            HighlightCurrentSelectable();
            StopAutoMove();
            return;
        }
        ...
        void tryMove()
        {
            ...
            //print($"{_currentSelectable.gameObject.name} -> " + moveSelectable.gameObject.name);
    ...
    protected virtual void Update()
    {
        ...
        _justActivated = false;
        HighlightedGO = _currentHighlightedGO;
    }
    protected virtual void Awake()
    {
        //print("Awake: " + gameObject);
    ...
    {
        //Unregistering
        //print("Disabled: " + gameObject);

        _input.PauseButtonPerformed -= Back; 
        _input.Button1Performed -= OnSelectablePress;
        //print(_input?.Button1Performed.GetInvocationList().Length + "");
        _justActivated = false;
        _enabled = false;
    }
    ...
    GameObject _currentHighlightedGO { get { return EventSystem.current.currentSelectedGameObject; } 
        set 
        {
